...
- `todos`
  ...
  - `user_id` (uuid, not null, defaults to the authenticated user) — owner of the task
  - `created_at` (timestamptz, default now()) — creation timestamp
2. Security
- Enable RLS on `todos`.
- Owner-scoped CRUD: each authenticated user can only select, insert, update, and delete rows they own.
- `user_id` defaults to `auth.uid()` so inserts that omit the owner still satisfy the WITH CHECK policy.
3. Indexes
